...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'manager', 'employee', 'supplier', 'customer'];
  ...
  const { data, error, isLoading } = useSWR<BusinessInterface[]>(
    ...
    () => getBusinesses({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    { name: 'Invoices', path: '/invoices', entity: 'invoice', service: AccessServiceEnum.PROJECT, icon: FiFileText },
    { name: 'Orders', path: '/orders', entity: 'order', service: AccessServiceEnum.PROJECT, icon: FiShoppingCart },
    { name: 'Products', path: '/products', entity: 'product', service: AccessServiceEnum.PROJECT, icon: FiBox },
