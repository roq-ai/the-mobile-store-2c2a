...
import { OrderListPage as ShowCaseList } from 'pages/orders';
...
function DashboardPage() {
  ...
  const inventoryStat = useSWR<PaginatedInterface<InventoryInterface>>(
    () => `/inventories`,
    () => getInventories({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Inventories',
      result: inventoryStat?.data?.totalCount,
      isLoading: inventoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
