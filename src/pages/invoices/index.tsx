...
type ColumnType = ColumnDef<InvoiceInterface, unknown>;
...
export function InvoiceListPage(props: InvoiceListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getInvoices({
        ...
        order: params.order,
        searchTermKeys: ['payment_method.contains', 'invoice_status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<InvoiceInterface>>(
    () => `/invoices?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: InvoiceInterface) => {
    if (hasAccess('invoice', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/invoices/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'payment_method', header: 'Payment Method', accessorKey: 'payment_method' },
    { id: 'discount_amount', header: 'Discount Amount', accessorKey: 'discount_amount' },
    { id: 'invoice_status', header: 'Invoice Status', accessorKey: 'invoice_status' },
    !hideActions
      ? {
          id: 'actions',
          header: '',
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/invoices/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('invoice', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/invoices/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="invoice" />
        </Flex>

        {hasAccess('invoice', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/invoices/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
