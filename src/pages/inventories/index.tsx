...
type ColumnType = ColumnDef<InventoryInterface, unknown>;
...
export function InventoryListPage(props: InventoryListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<InventoryInterface>>(
    () => `/inventories?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: InventoryInterface) => {
    if (hasAccess('inventory', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/inventories/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    { id: 'quantity', header: 'quantity', accessorKey: 'quantity' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.last_updated ? format(parseISO(record?.last_updated as unknown as string), 'dd-MM-yyyy') : '',
    },
    hasAccess('product', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'product',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/products/view/${record.product?.id}`}>
              {record.product?.name}
            </Link>
          ),
        }
