...
export function OrderListPage(props: OrderListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<OrderInterface>>(
    () => `/orders?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.delivery_date ? format(parseISO(record?.delivery_date as unknown as string), 'dd-MM-yyyy') : '',
    },
