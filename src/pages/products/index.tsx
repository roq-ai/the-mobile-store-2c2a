...
export function ProductListPage(props: ProductListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getProducts({
        relations: ['business', 'inventory.count', 'order.count', 'review.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['name.contains', 'description.contains', 'category.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ProductInterface>>(
    () => `/products?params=${JSON.stringify(params)}`,
    fetcher,
  );
