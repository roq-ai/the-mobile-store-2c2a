...
export function BusinessListPage(props: BusinessListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getBusinesses({
        relations: ['user', 'inventory.count', 'product.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BusinessInterface>>(
    () => `/businesses?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('product', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Product',
          accessorKey: 'product',
          cell: ({ row: { original: record } }: any) => record?._count?.product,
        }
      : null,
